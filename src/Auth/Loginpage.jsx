...
const Loginpage = () => {
    ...
    const handleLogin = async () => {
        ...
        try {
            const response = await fetch('https://api-dev.blue-nodes.app/dev/auth/login', {
                method: 'POST',
                headers: {
    ...
    return (
        <div className='w-full h-screen flex flex-row justify-center items-center'>
            <div className='w-[50%] h-full flex flex-col justify-center items-center gap-5 px-24 bg-[#F1F1F1]'>
                ...
                <h3 className='text-lg text-gray-700'> Welcome back! Please enter your details. </h3>
                ...
                <div className='w-full flex flex-row justify-between items-start'>
                    ...
                    <h2 className='text-[#0DC8ED] text-base cursor-pointer'> Forget Password </h2>
                </div>
                <button
                    onClick={handleLogin}
                    className={`w-full py-2 rounded-[10px] bg-[#0BAAC9] text-base font-semibold text-white ${loading ? 'opacity-50 cursor-not-allowed' : ''}`}
                    disabled={loading}
                >
                ...
                <div className='w-full flex flex-row justify-center items-start gap-1'>
                    ...
                    <h2 className='text-[#0DC8ED] text-base cursor-pointer font-semibold'> Sign up </h2>
                </div>
            </div>
