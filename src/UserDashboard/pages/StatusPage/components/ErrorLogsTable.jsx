...
const ErrorLogsTable = () => {
  ...
  const transformData = (nodes) => {
    return nodes.map((node) => {
      const key = node.children.length > 0 ? node.id.toString() : `room${node.id}`;
  ...
  const updateSelection = (newSelectedKeys) => {
    ...
    const transformedArray = locations.map((item) => parseInt(item.replace("room", ""), 10));
  ...
  const getData = async (locations) => {
    try {
      const data = await fetchEventLogsData(currentPage, itemsPerPage, locations, "Error", dateTo, dateFrom);
  ...
  }, [ApiLocationsToBeSend, dateTo, dateFrom]);
